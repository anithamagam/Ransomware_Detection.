...
def print_evaluation_metrics(evaluation_metrics):
    ...
    print("\n--------------------- Distance Deviations ---------------------\n")
    print(
        "Accuracy: ", distance_deviations[0] if len(distance_deviations) > 0 else "N/A"
    )
    print(
        "Confusion Matrix: ",
        "| TN:",
        distance_deviations[1] if len(distance_deviations) > 1 else "N/A",
        "| FP:",
        distance_deviations[2] if len(distance_deviations) > 2 else "N/A",
        "| FN:",
        distance_deviations[3] if len(distance_deviations) > 3 else "N/A",
        "| TP:",
        distance_deviations[4] if len(distance_deviations) > 4 else "N/A",
        "|",
    )
    print(
        "Precision: ", distance_deviations[5] if len(distance_deviations) > 5 else "N/A"
    )
    print("Recall: ", distance_deviations[6] if len(distance_deviations) > 6 else "N/A")
    print(
        "F1-score: ", distance_deviations[7] if len(distance_deviations) > 7 else "N/A"
    )
    print(
        "ROC-AUC: ", distance_deviations[8] if len(distance_deviations) > 8 else "N/A"
    )
    print("\n-------------------------------------------------------------\n")
...
def get_evaluation_metrics_means(evaluation_metrics):
    ...
    confusion_matrices: list[np.ndarray] = evaluation_metrics[1]
    ...
    means = [
        ...
        np.array(sum(confusion_matrices) / len(confusion_matrices)).tolist(),
