...
def feature_extraction(apk_path, most_relevant_features):
    ...
    intents = (
        activities_intents + services_intents + receivers_intents + providers_intents
    )
    ...
    for required_feature in most_relevant_features:
        ...
        if not match_found:
            extraction_result.append(
                0
            )  # Append 0 if no match is found for the current feature
