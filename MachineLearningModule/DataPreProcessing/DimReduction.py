...
def relevance_measure_mean_median(dataset):
    ...
    df = pd.DataFrame(arr_mm, index=pd.Index(arr_mm_features))
...
def relevance_measure_fisher_ratio(dataset, class_name):
    ...
    for feature in features:
        ...
        feature_mean_for_class0 = np.mean(
            dataset.loc[
                dataset.index[dataset[class_name] == class_name_values[0]].tolist()
            ][feature]
        )
        feature_mean_for_class1 = np.mean(
            dataset.loc[
                dataset.index[dataset[class_name] == class_name_values[1]].tolist()
            ][feature]
        )
    ...
    df = pd.DataFrame(arr_fr, index=pd.Index(arr_fr_features))
    features = cumulative_relevance(df)
